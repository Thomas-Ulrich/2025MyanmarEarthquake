...
for idx, (lon1, lat1, lon2, lat2) in enumerate(segments):
    ...
    cos_dip = np.cos(np.radians(common_segment_props["dip"]))
    sin_dip = np.sin(np.radians(common_segment_props["dip"]))
    delta_dip = common_segment_props["delta_dip"]
    hyp_dip = int(np.round(10 / (sin_dip * delta_dip)))
    ...
    if idx == northernmost_idx:
        ...
        yrel = (yt - y2) / (y1 - y2)
        hyp_stk = int(np.round(yrel * stk_subfaults)) + 1
        # fault strike ~=0 for this fault
        xi = x2 + yrel * (x1 - x2) + (hyp_dip - 0.5) * cos_dip * delta_dip * 1000
        yi = y2 + yrel * (y1 - y2)
...
connections = []
for idx, (x1, y1, x2, y2) in enumerate(segments[:-1]):
    print(idx)
    # Create configuration per segment
    connections.append(
        {
            "segment1": idx + 1,
            "segment1_subfault": [1, 1],
            "segment2": idx + 2,
            "segment2_subfault": [stk_subfaults_all[idx + 1] + 1, 1],
        }
    )
...
result = {
    "rise_time": {"delta_rise": 2.5, "min_rise": 2.5, "windows": 9},
...
for idx, (x1, y1, x2, y2) in enumerate(segments):
    ...
    config0 = {}
    config0 |= config
    neighbour_right = {"segment": idx, "subfault": 1} if idx > 0 else None
    neighbour_left = (
