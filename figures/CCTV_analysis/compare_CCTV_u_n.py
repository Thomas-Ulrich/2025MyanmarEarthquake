...
for i, fname in enumerate(rec_files):
    ...
    plt.plot(frd.Time, frd.u_n)
    plt.plot(frd.Time, np.log10(np.abs(frd.u_n)))
    plt.ylim([-6, 4])
...
for k, modeli in enumerate(top10_indices[::-1]):
    ...
    #ax.plot(time_shifted, frd.u_n[id0:], linestyle="-", color=line.get_color())
    ids = np.where(time_shifted>-10)[0][0]
    time_shifted = frd.Time[ids:] - t0
    ax.plot(time_shifted, np.log10(np.abs(frd.u_n[ids:])), linestyle="-", color='r')
