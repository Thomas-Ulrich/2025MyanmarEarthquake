...
ax2 = ax1.twinx()
ax2.set_ylabel("Fault slip (m)", color='tab:orange')
...
for i, fname in enumerate(rec_files):
    ...
    if args.plot_all:
        ...
        ax1.plot(time_shifted, sr_shifted, label=f"{sim_id}")
...
for k, modeli in enumerate(top10_indices[::-1]):
    ...
    if not args.plot_all:
        ...
        # Plot preferred model
        ax1.plot(time_shifted, sr_shifted, label=f"{sim_id}, {float(wrms[modeli]):.2f}")
        # Compute slip (integrated slip rate)
        slip = cumtrapz(sr_shifted, time_shifted, initial=0)

        # Plot slip on the same plot with a secondary y-axis, or optionally on a separate figure
        ax2.plot(time_shifted, slip, label=f"{sim_id}, Slip", linestyle='--')
